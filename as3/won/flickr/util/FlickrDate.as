package won.flickr.util
{
	public class FlickrDate
	{
		public static function convert($date:Number, $format:String="Y.m.d"):String
		{
			var date:Date = new Date($date * 1000);			
			...
			if ($format.indexOf('Y') != -1) $format = getFullYear($format, y); //2009
			if ($format.indexOf('y') != -1) $format = getShortYear($format, y); //09
			if ($format.indexOf('M') != -1) $format = getRegMonth($format, m); //3
			if ($format.indexOf('m') != -1) $format = getZeroMonth($format, m); //03
			if ($format.indexOf('D') != -1) $format = getRegDay($format, d); //5
			if ($format.indexOf('d') != -1) $format = getZeroDay($format, d); //05			
